...
#
"""
本文件主要改rdp_check函数中，如果连到目标机器帐号密码认证失败从logging.error('Access Denied')改为认证失败后return False,认证成功
return True
s.socket(ip,3389)改成了s.socket(ip,dport)
"""
import socket
import argparse
import sys
...
def check_rdp(host,dport,username, password, domain, hashes=None):
    ...
    try:
        ...
        """
        当rdp连接成功后并且认证也成功，目标服务端不会有响应，认证失败会抛出异常Access Denied，这里我改成return False
        """
        buff = tls.recv(1024)
    except Exception as err:
        if str(err).find("denied") > 0:
            #print(err)
            #print(str(err).find("denied"))
    ...
    #ts_re=ts_request.getData()
    #print(ts_re)
    tls.send(ts_request.getData())
    tls.close()
    logging.info("Access Granted")
    return True
    #这个认证过程通过后关闭tcp连接，logging日志信息中写入访问成功，因为之前说了认证成功后服务器端不会有回答，所以没有异常报错就是认证成功
    # If auth is successful, the server waits for the pubKeyAuth and doesn't answer
    #print(logging.info("Access Granted"))


#print(check_rdp('192.168.89.131', 'administrator', 'P@ssw0rd', ''))
...
def main():
    ...
    #check_rdp('192.168.89.130','administrator','P@s1sw0rd','')

main()

#res=check_rdp('192.168.89.130','administrator','P@ssw0rd','')
#print(res)
"""